...
class SecondActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnChoose.setOnClickListener {
            ...
            startActivityForResult( thirdActivityIntent, REQUEST_CODE)
        }
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_CODE) {
            if (resultCode == Activity.RESULT_OK) {
                @Suppress("DEPRECATION")
                selectedDataItem = data?.getSerializableExtra("selected_user") as DataItem?
                binding.tvSelected.text = getString(
                    R.string.user_name_format,
                    selectedDataItem?.firstName,
                    selectedDataItem?.lastName
                )
            }
        }
    }
